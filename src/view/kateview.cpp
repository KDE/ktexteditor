    ...
    , m_autoFoldedFirstLine(false)
{
    ...
    registerCompletionModel(EditorPrivate::self()->modelineCompletionModel());
}
...
void KTextEditor::ViewPrivate::setupSpeechActions()
{
    ...
    QAction *a = ac->addAction(QStringLiteral("tools_speech_say"));
...
bool KTextEditor::ViewPrivate::evaluateScriptInternal(const QString &script, QVariant *result)
{
    ...
    if (!kateScript.load()) {
        if (result) {
            *result = kateScript.errorMessage();
        }
        return false;
    }
    ...
    if (result) {
        *result = res.isError() ? res.toString() : res.toVariant(QJSValue::ConvertJSObjects);
    }
    return (!res.isError());
}
...
KTextEditor::Cursor KTextEditor::ViewPrivate::coordinatesToCursor(const QPoint &coords) const
{
    ...
    return m_viewInternal->coordinatesToCursor(m_viewInternal->mapFromParent(coords), KateViewInternal::CursorForCoordinatesOutsideText::Invalid, false);
}
...
const QTextLayout *KTextEditor::ViewPrivate::textLayout(const KTextEditor::Cursor pos) const
{
...
{
    if (blockSelect && selection()) {
        for (int line = selectionRange().start().line(); line <= selectionRange().end().line(); line++) {
            KTextEditor::Cursor c(line, 0);
            KTextEditor::Range r = KTextEditor::Range(c, c);
            doc()->indent(r, 1);
        }
    } else {
...
QList<KTextEditor::AttributeBlock> KTextEditor::ViewPrivate::lineAttributes(int line)
{
    ...
    const Kate::TextLine kateLine = doc()->kateTextLine(line);
    const auto &intAttrs = kateLine.attributesList();
    for (qsizetype i = 0; i < intAttrs.size(); ++i) {
