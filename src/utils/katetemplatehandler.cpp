...
void KateTemplateHandler::jumpToPreviousRange()
{
    ...
    foreach (MovingRange *range, m_masterRanges) {
        ...
        if (!previousRange || range->start() > previousRange->start()) {
            ...
            if (m_templateRangesChildToParent.value(previousRange)) {
                previousRange = m_templateRangesChildToParent.value(previousRange);
            }
        }
    }
...
void KateTemplateHandler::jumpToNextRange()
{
    ...
    if (nextRange) {
        if (m_templateRangesChildToParent.value(nextRange)) {
            nextRange = m_templateRangesChildToParent.value(nextRange);
        }
