var katescript = {
    "name": "Lua",
    "author": "Jonathan Schmidt-Dominé <devel@the-user.org>",
    "license": "LGPL",
    "revision": 2,
    "kate-version": "5.1"
...
var triggerCharacters = "defl})";

// Indent after lines that match this regexp
var rxIndent = /(\bfunction\s|(\b(else|then|do)\s*$)|([\{][^\}]*$)|([\(][^\)]*$))/;

// Unindent lines that match this regexp
var rxUnindent = /^\s*([\}\)]|(end|else|elseif|until)\b)(.*)$/;
...
function isStmtContinuing(line)
{
  ...
  var rx = /(\+|\-|\*|\/|\%|\^|\=|\<|\>|\band\b|\bor\b|\bnot\b|#|,|[^\.]\.\.|^\.\.|[^\.]\.|^\.)\s*/g;
...
function isBlockStart(stmt)
{
  ...
  var rx = /((\b(do|then|else)\b)|\bfunction\s+\([\w\,]*\))/g;
